...
const MyFavouriteCard = ({ artwork, onRemove }) => {
  ...
  const handleRemoveFav = () => {
    fetch(
      `https://artify-server-one.vercel.app/favourites/${artwork._id}?userEmail=${user.email}`,
      {
        method: "DELETE",
      },
    )
      ...
      .then((data) => {
        if (data.deletedCount) {
          toast.success(
            "Content is removed successfully from your favourites!",
          );
