...
const MyGallery = () => {
    ...
    useEffect(()=>{
        fetch(`http://localhost:3000/myArtworks/?email=${user.email}`)
        .then(res=>res.json())
        .then((data)=>{
           setArtworks(data);
        })
    ...
    return (
        <div className='w-11/12 mx-auto'>
            ...
            {
               artworks.map(artwork=><MyGalleryCard key={artwork._id} artwork={artwork} onRemove={removeFromUI}></MyGalleryCard>)
            }
            </div>
        </div>
    );
